<template>
  <div class="div-class" v-if="orderObj != undefined">
    <el-row style="padding: 8px">
      ...
      <el-col :offset="18" :span="4" style="font-size: 20px;">订单号：<sapn style="font-size: 20px;color: red;"></sapn>{{orderObj.odrOn}}</el-col>
    </el-row>
        ...
        <el-col>
          <el-table
            ...
            <el-table-column prop="odrdlCount"   label="数量" >
              <template #default="obj">
                <el-input-number size="mini" :min="1" :max="999999" v-model="obj.row.odrdlCount"  placeholder="产品数量" type="text" />
              </template>
            </el-table-column>
    ...
    <el-row>
      <el-col :offset="11"  :span="2">
        <el-button  @click="addProductDetails"  size="mini" type="primary">保存明细数据</el-button>
      </el-col>
  ...
  <div class="div-class" style="position: absolute;width: 100%;height: 100%" v-if="orderObj == undefined">
    错误！！！！！！！！
...
export default {
  data(){
    return{
      orderObj:JSON.parse(sessionStorage.getItem('orderObj')),//订单对象
  ...
  methods:{
    initProduct(){
      ...
      if(this.orderObj.orderFromDetail.length != 0) {
        for (let i = 0; i < this.orderObj.orderFromDetail.length; i++) {
          ...
          orderDetailsDx.odrId = this.orderObj.orderFromDetail[i].odrId;//订单号
    ...
    addProductDetails(){
      ...
      this.axios.post('orderFromDetail/addBatchOrderDetails',{"orderDetails":this.orderDetailsArr,"odrOn":this.orderObj.odrOn,"odrId":this.orderObj.odrId}).then((v)=>{
          ElMessage.success({
            message: "保存成功",
            type: 'success'
          });
    ...
    addProductFun(obj){
      ...
      orderDetailsDx.odrId = this.orderObj.odrId;//订单编号
      orderDetailsDx.prId = obj.prId;//产品编号
      orderDetailsDx.odrdlProductName = obj.prName;//产品名称
      orderDetailsDx.odrdlMoney = obj.prCostPrice;//进价
      orderDetailsDx.odrdlUnitPrice = obj.prPrice;//售价
